...
import AddDiscountFormAlert from './alerts/AddDiscountFormAlert';
import ReactivateDiscountFormAlert from './alerts/ReactivateDiscountFormAlert';
...
function CreateDiscountForm({ storeId }: { storeId: string }) {
  const router = useRouter();
  const searchParams = useSearchParams();
  const status = searchParams?.get('status');
  const { data: session, update } = useSession();
  const [disabled, setDisabled] = useState(false);
  const [openDiscountRecovery, setOpenDiscountRecovery] = useState(false);
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        ...
        if (response) {
          toast.success(response.message || 'Discount successfully created!');
          router.push(
            `/dashboard/discounts/store/${storeId}/new?status=successful`,
          );
        } else {
  ...
  return (
    <form className="flex flex-col gap-4" onSubmit={formik.handleSubmit}>
      ...
      <Button
        ...
        onClick={() => {}}
      >
        {disabled ? 'Adding Discount' : 'Add Discount'}
      </Button>
      <Button
        variant="link"
