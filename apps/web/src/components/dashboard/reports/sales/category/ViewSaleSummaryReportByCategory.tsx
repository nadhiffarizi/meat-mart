...
function ViewSaleSummaryReportByCategory({ storeId }: { storeId: string }) {
  ...
  useEffect(() => {
    async function getAllTransactionDetailSummary() {
      try {
        const response = await api(
          `dashboard/transactionDetail/summary?storeId=${storeId}&month=${month}&year=${year}&groupBy=category`,
          'GET',
          {},
          session?.user.access_token,
        );
...
export default ViewSaleSummaryReportByCategory;
