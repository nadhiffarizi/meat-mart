...
export const subtractCartAPI = async (
  apiRouter: string,
  payload: payloadCartService,
) => {
  const response = await apiRequest(
...
export const indexProductInCart = (cartState: ICart[], product: IProduct) => {
  ...
  const indexProductInCart = cartState.findIndex(
    (cartData) => cartData.product.id === product.id,
  );
...
export const isMaxAddedToCart = (
  ...
  if (
    ...
    maxStockAvailable(productFromDB).maxStockAvailable - 1
  ) {
    return true;
  } else {
    return false;
...
export const whichStockApplied = (productFromDB: IProduct): IStock => {
  // find branch store
  const findIndexBranch = productFromDB.availableStocks.findIndex(
    (stock) => stock.stores.status === 'BRANCH',
  );
  if (findIndexBranch == -1) {
    // cannot get branch store, refer to CENTRAL
...
export const countCartTotalPrice = (cartState: ICart[]): number => {
  let tempTotal = 0;
  cartState.map(
    (cartItem) => (tempTotal += cartItem.product.price * cartItem.quantity),
  );
  return tempTotal;
};
