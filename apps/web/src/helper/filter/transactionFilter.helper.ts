...
export const setQueryParams = (filterTransactions: IFilterTransactions | undefined) => {
    ...
    if (filterTransactions.invoiceNumber && filterTransactions.invoiceNumber?.length > 0) {
        params.append('invoice', filterTransactions.invoiceNumber)
    }

    if (filterTransactions.statusArray && filterTransactions.statusArray?.length > 0) {
