...
const cartSlice = createSlice({
  ...
  reducers: {
    ...
    addToCartState: (
      state: ICart[],
      action: PayloadAction<{ product: IProduct; qtty: number }>,
    ) => {
      // temp state
      ...
      if (indexProductInCart(tempState, action.payload.product) == -1) {
        ...
        if (
          ...
          maxStockAvailable(action.payload.product).maxStockAvailable
        ) {
    ...
    subtractCartState: (
      ...
      const indexProduct = indexProductInCart(
        tempState,
    ...
    addDiscountToCartItem: (
      state: ICart[],
      action: PayloadAction<{ data: any; cartId: string }>,
    ) => {
      // temp state
      const tempState = [...state];
