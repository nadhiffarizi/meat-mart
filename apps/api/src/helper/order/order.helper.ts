...
export const updateCartToOrder = async (
  ...
  for (let cartItem of carts) {
    ...
    if (cartItem.quantity > currentStock?.quantity!) {
      ...
      const chosenStockId = chooseStock(
        availableStocks,
        cartItem.quantity,
        cartItem,
        'UPDATE',
...
export const updateOrderStatusByTrxId = async (
  trxId: string,
  statusToBe: E_OrderStatus,
) => {
