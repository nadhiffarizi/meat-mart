...
export const calculateAfterDisc = (
  ...
  promotionType: E_PromotionType,
  discount: IDiscount,
  existingCart: ICart,
): ICartAfterDIsc | undefined | number => {
  switch (promotionType) {
    case E_PromotionType.CUSTOM:
      const cartAfterDisc1 = priceAfterCustomDisc(
    ...
    case E_PromotionType.BOGO:
      const cartAfterDisc2 = processBOGODisc(
        defaultPricePerProduct,
        existingCart!,
      );
    ...
    case E_PromotionType.MINIMUM_BUY:
      const cartAfterDisc3 = priceAfterMinBuy(
        defaultPricePerProduct,
        discount,
        existingCart!,
      );
...
export const priceAfterCustomDisc = (
  defaultPricePerProduct: number,
  discount: IDiscount,
  existingCart: ICart,
) => {
  if (discount.discount_amount) {
    ...
    let pricePerProductAfterDisc =
      defaultPricePerProduct - discount.discount_amount;
    ...
    const amountDiscounted =
      discount.discount_percentage * 0.01 * defaultPricePerProduct;
...
export const processBOGODisc = (
  ...
  const feedBackDiscount: ICartAfterDIsc = {
    ...
    pricePerProduct: parseFloat(
      (defaultPricePerProduct / (existingCart.quantity * 2)).toFixed(3),
    ),
  };
  return feedBackDiscount;
};

export const priceAfterMinBuy = (
  defaultPricePerProduct: number,
  discount: IDiscount,
  existingCart: ICart,
) => {
  ...
  if (discount.discount_amount) {
    ...
    const pricePerProduct = parseFloat(
      (totalPriceAfterDisc / existingCart.quantity).toFixed(3),
    );
    ...
    let amountDiscounted =
      totalDefaultPrice * 0.01 * discount.discount_percentage!; // in rupiah
...
export const findDiscountsByStockId = async (
  stockId: string,
  existingCart: ICart,
) => {
  // find stock by id
