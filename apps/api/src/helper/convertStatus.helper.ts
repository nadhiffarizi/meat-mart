...
export const convertTransactionStatusToEnum = (status: string[]): E_TransactionStatus[] => {
    if (!status || status.length === 0) {
        return [E_TransactionStatus.AWAITING_PAYMENT, E_TransactionStatus.CANCELED, E_TransactionStatus.DONE, E_TransactionStatus.CONFIRMED_ADMIN, E_TransactionStatus.PENDING_ADMIN]
    } else {
