...
class TransactionService {
  async create(req: Request) {
    try {
      ...
      if (role !== E_Role.CUSTOMER)
        throw new Error('Unauthorized role to access this service');
      ...
      // find carts data
      const carts = await findCartByOrderInput(
        orderInputs as IOrderInput[],
        user?.id!,
      );

      // update cart to order
      const filteredCarts = await updateCartToOrder(
        carts as ICart[],
        user?.id!,
        loc1,
      );
      ...
      if (filteredCarts.notEligibleCart.length !== 0) {
        ...
        return returnServiceFeedback(
          406,
          filteredCarts.notEligibleCart,
          statusEnum.FAILED,
          'Transaction failed to create, there are insufficient stock in some of your carts',
        );
      } else {
        ...
        for (let i = 0; i < filteredCarts.eligibleCart.length; i++) {
          ...
          const trxDetail = await createTrxDetails(
            trxId,
            cartItem,
            loc1,
            (orderInputs as IOrderInput[])[i],
          );
        ...
        if ((method as string).toUpperCase() === 'AUTOMATIC') {
          snapResponse = {
            ...(await midtransSnap(
              { id: newTrx.id, gross_amount: newTrx.total_price! },
              user!,
            )),
          };
        }
      ...
      // get cart
      const cartAfterCheckout = await findCartByIds(
  ...
  async createMidtrans(req: Request) {
    // for example
    const requestTrx = await midtransSnap({
      gross_amount: 30000,
      id: 'diuwhdniqowdqwe',
    });
  ...
  async handleNotifMidtrans(req: Request) {
    ...
    try {
      ...
      if (notification['status_code'] !== '200')
        throw new Error(`${notification['status_code']}`);
      let data = {};
      if (transactionStatus == 'capture' || transactionStatus == 'settlement') {
        if (notification['fraud_status'] === 'accept') {
          ...
          const updatedTrx = await updateTrxStatus(
            trxId,
            E_TransactionStatus.CONFIRMED_ADMIN,
          );
          // update orders
          const updatedOrders = await updateOrderStatusByTrxId(
            trxId,
      ...
      return returnServiceFeedback(
        ...
        { ...notification, ...data },
        statusEnum.SUCCESS,
        'payment midtrans successful',
      );
    } catch (error) {
      return returnServiceFeedback(
        Number((error as Error).message[0]),
        null,
        statusEnum.FAILED,
  ...
  async uploadTrxProof(req: Request) {
    ...
    try {
      ...
      return returnServiceFeedback(
        ...
        updatedTrx,
        statusEnum.SUCCESS,
        'upload transaction proof success',
      );
    } catch (error) {
      return returnServiceFeedback(
        400,
        (error as Error).message,
        statusEnum.FAILED,
        'upload transaction proof failed',
      );
    }
  }
}
