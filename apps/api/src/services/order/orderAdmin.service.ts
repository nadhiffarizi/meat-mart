...
import { updateOrderStatusById, updateOrderStatusByTrxId } from "@/helper/order/order.helper"
import { getOrderById, getOrderByTrxId } from "@/helper/order/orderQuery.helper"
...
class OrderServiceAdmin {
    async cancelOrderAdmin(req: Request) {
        ...
        try {
            ...
            if (order?.status === E_OrderStatus.ON_DELIVERY || order?.status === E_OrderStatus.CONFIRMED || !order) throw new Error("Unauthorized access. Order cannot be canceled")

            // update status order by admin
            const updatedOrder = await updateOrderStatusById(orderId, E_OrderStatus.CANCELED)
            if (!updatedOrder) throw new Error("No order data satisfied with the id")

            // return stock
            const returnedStock = await updateStockQuantity(updatedOrder.stock_id, updatedOrder.quantity, 'ADD')

            // check order details status in a transaction
            const ordersInTrx = await getOrderByTrxId(updatedOrder.transaction_id)
            if (ordersInTrx.every((order) => order.status === E_OrderStatus.CANCELED)) {
                // update transaction status
                await updateTrxStatus(updatedOrder.transaction_id, "CANCELED")
            }

            // feedback from service
            return returnServiceFeedback(200, { updatedOrder, ordersInTrx }, statusEnum.SUCCESS, "get order list success")
